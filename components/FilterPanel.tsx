...
export default function FilterPanel({ filters, onChange, onReset, markets, marketLabels, marketColors, cuisines, companions, total, shown }: Props) {
  ...
  return (
    <div className="space-y-5 text-sm">
        ...
        <div className="space-y-1">
          {markets.map(m => (
            <label key={m} className="flex items-center gap-2 cursor-pointer group">
              ...
              <span className="text-gray-300 group-hover:text-white">{marketLabels[m]}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div>
        <p className="text-gray-400 text-xs font-medium uppercase mb-2">Companion</p>
      ...
      <div>
        ...
        <div className="flex gap-2">
          {[['', 'All'], ['男性', 'Male'], ['女性', 'Female']].map(([v, label]) => (
            <button key={v} onClick={() => set({ sex: v })} className={`flex-1 py-1 rounded-lg text-xs ${filters.sex === v ? 'bg-red-500 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'}`}>{label}</button>
          ))}
        </div>
      </div>
      <div>
